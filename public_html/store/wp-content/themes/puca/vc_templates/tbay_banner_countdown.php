	...
	<div class="inner text-center space-padding-tb-30" <?php echo trim($fstyle); ?>>
		...
		<?php endif; ?>	
		 ...
		 <div class="countdown-wrapper">
		    <div class="tbay-countdown" data-id="<?php echo esc_attr($_id); ?>" id="countdown-<?php echo esc_attr($_id); ?>" data-countdown="countdown scroll-init" data-days="<?php esc_attr_e('Days','puca'); ?>" data-hours="<?php esc_attr_e('Hours','puca'); ?>"  data-mins="<?php esc_attr_e('Mins','puca'); ?>" data-secs="<?php esc_attr_e('Secs','puca'); ?>" data-date="<?php echo gmdate('m',$time).'-'.gmdate('d',$time).'-'.gmdate('Y',$time).'-'. gmdate('H',$time) . '-' . gmdate('i',$time) . '-' .  gmdate('s',$time) ; ?>">
		    </div>
		</div>

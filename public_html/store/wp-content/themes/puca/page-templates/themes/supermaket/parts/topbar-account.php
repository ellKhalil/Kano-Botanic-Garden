	...
	<div class="tbay-login">	
		...
		<?php } elseif( !(defined('PUCA_WOOCOMMERCE_CATALOG_MODE_ACTIVED') && PUCA_WOOCOMMERCE_CATALOG_MODE_ACTIVED) && defined('PUCA_WOOCOMMERCE_ACTIVED') && PUCA_WOOCOMMERCE_ACTIVED && !empty(get_option('woocommerce_myaccount_page_id')) ) { ?>  
				<?php if( puca_tbay_get_config('enable_popup_login', false) ) : ?>
					<a href="javascript:void(0)" title="<?php esc_attr_e('Sign me up','puca'); ?>" data-toggle="modal" data-target="#custom-login-wrapper">
						<span class="account-button"><i class="sim-icon icon-user"></i></span>
						<span class="well-come"><?php esc_html_e('Hello,','puca'); ?></span>
						<span class="account-name"><?php esc_html_e('Sign me up','puca'); ?></span>
					</a>        
				<?php else: ?> 
					<a href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>" title="<?php esc_attr_e('Sign me up','puca'); ?>">
						<span class="account-button"><i class="sim-icon icon-user"></i></span>
						<span class="well-come"><?php esc_html_e('Hello,','puca'); ?></span>
						<span class="account-name"><?php esc_html_e('Sign me up','puca'); ?></span>
					</a>        

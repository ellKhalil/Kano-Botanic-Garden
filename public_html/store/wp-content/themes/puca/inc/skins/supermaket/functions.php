...
if ( !function_exists('puca_tbay_widgets_init') ) {
	function puca_tbay_widgets_init() {
		...
		if ( class_exists('SitePress') ) {
			register_sidebar( array(
				'name'          => esc_html__( 'WPML Sidebar', 'puca' ),
				'id'            => 'wpml-sidebar',
				'description'   => esc_html__( 'Add widgets here to appear.', 'puca' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h2 class="widget-title">',
				'after_title'   => '</h2>',
...
//Check active WPML
if ( !function_exists('puca_tbay_wpml') ) {
	function puca_tbay_wpml() {
		if(is_active_sidebar('wpml-sidebar')) {
			dynamic_sidebar('wpml-sidebar'); 
		}
	}

	add_action('puca_tbay_header_custom_language','puca_tbay_wpml', 10);
}

...
if ( !function_exists('puca_tbay_get_header_layout') ) {
	function puca_tbay_get_header_layout() {
		...
		if ( is_page() && is_object($post) && isset($post->ID) ) {
			...
			return puca_tbay_woo_get_header_layout( wc_get_page_id( 'shop' ) );
		} else if( class_exists( 'WooCommerce' ) && is_cart() ) {
			return puca_tbay_woo_get_header_layout( wc_get_page_id( 'cart' ) );
		} else if( class_exists( 'WooCommerce' ) && is_checkout() ) {
			return puca_tbay_woo_get_header_layout( wc_get_page_id( 'checkout' ) );
		}
...
if ( !function_exists('puca_tbay_woo_get_header_layout') ) {
	function puca_tbay_woo_get_header_layout( $page_id ) {
		$header = get_post_meta( $page_id, 'tbay_page_header_type', true );

		if ( $header == 'global' ||  $header == '') {
			return puca_tbay_get_config('header_type', 'header_default');
		} else {
			return $header;
		}
	}
...
if ( !function_exists('puca_tbay_get_footer_layout') ) {
	function puca_tbay_get_footer_layout() {
		if ( is_page() ) {
			...
			return puca_tbay_woo_get_footer_layout( wc_get_page_id( 'shop' ) );
		} else if( class_exists( 'WooCommerce' ) && is_cart() ) {
			return puca_tbay_woo_get_footer_layout( wc_get_page_id( 'cart' ) );
		} else if( class_exists( 'WooCommerce' ) && is_checkout() ) {
			return puca_tbay_woo_get_footer_layout( wc_get_page_id( 'checkout' ) );
		}
...
if ( !function_exists('puca_tbay_woo_get_footer_layout') ) {
	function puca_tbay_woo_get_footer_layout( $page_id ) {
		$footer = get_post_meta( $page_id, 'tbay_page_footer_type', true );

		if ( $footer == 'global' ||  $footer == '') {
			return puca_tbay_get_config('footer_type', 'footer_default');
		} else {
			return $footer;
		}
	}

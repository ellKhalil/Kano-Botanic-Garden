...
if ( ! function_exists( 'puca_fashion3_tbay_icon_post_formats' ) ) {
	function puca_fashion3_tbay_icon_post_formats() {
			$post_id 			= get_the_ID();
			$post_format 	= get_post_format($post_id);

			switch ($post_format) {
				case 'aside':
					echo '<i class="tb-icon tb-icon-zz-img-post-2"></i>';
					break;				
				case 'audio':
					echo '<i class="icon-music-tone-alt icons"></i>';
					break;				
				case 'gallery':
					echo '<i class="tb-icon tb-icon-zz-img-post-3"></i>';
					break;	
				case 'image':
					echo '<i class="tb-icon tb-icon-zz-img-post-1"></i>';
					break;			
				case 'link':
					echo '<i class="icon-link icons"></i>';
					break;				
...
if ( ! function_exists( 'puca_search_title_query' ) ) {
	function puca_search_title_query( $where, $wp_query ) {
	    if ( ! empty( $where ) && ! empty( $wp_query->query_vars['search_terms'] ) ) {
	        global $wpdb;
	        ...
	        foreach ( ( array ) $q['search_terms'] as $term )
	            $where[] = $wpdb->prepare( "$wpdb->posts.post_title LIKE %s", $n . $wpdb->esc_like( $term ) . $n );
...
if (! function_exists('puca_product_search_title')) {
    function puca_product_search_title($where, $wp_query)
    {
        if ( !defined('TBAY_FRAMEWORK_REDUX_ACTIVED') ) return $where;

        $where = puca_search_title_query($where, $wp_query);
 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'puca_init_product_search' ) ) {
	function puca_init_product_search() {
        if( puca_is_woocommerce_activated() ) {
            if ( apply_filters( 'puca_search_query_in', puca_tbay_get_global_config('search_query_in', 'title') === 'all' ) ) {
                add_filter( 'posts_search', 'puca_product_search_sku', 9 );
            } else {
                add_filter('posts_search', 'puca_product_search_title', 20, 2);
            }
        }
	}
	add_action( 'init', 'puca_init_product_search', 10 ); 
}

if ( ! function_exists( 'puca_product_search_sku' ) ) {
	function puca_product_search_sku( $where ) {
        if ( !defined('TBAY_FRAMEWORK_REDUX_ACTIVED') ) return $where;
...
if ( ! function_exists( 'puca_sku_search_query' ) ) {
	function puca_sku_search_query( $where, $s ) {
		global $wpdb;

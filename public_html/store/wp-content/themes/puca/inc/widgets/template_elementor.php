...
class Tbay_Widget_Template_Elementor extends Tbay_Widget
{
    ...
    {
        parent::__construct(
            'puca_template_elementor',
            esc_html__('Puca Template Elementor', 'puca'),
            [
                'description' => esc_html__('Embed your saved elements', 'puca'),
            ]
        ); 
    }
    ...
    public function form($instance)
    {
        ...
        if (! $templates) {
            ...
            <div class="tbay-elementor-widget-template-empty-templates-title">' . esc_html__('You Haven’t Saved Templates Yet.', 'puca') . '</div>
            <div class="tbay-elementor-widget-template-empty-templates-footer">' . esc_html__('Want to learn more about Elementor library?', 'puca') . ' <a class="elementor-widget-template-empty-templates-footer-url" href="https://go.elementor.com/docs-library/" target="_blank">' . esc_html__('Click Here', 'puca') . '</a>
            </div>
            </div>';
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('template_id')); ?>"><?php esc_html_e('Choose Template', 'puca'); ?>:</label>
			<select class="widefat elementor-widget-template-select" id="<?php echo esc_attr($this->get_field_id('template_id')); ?>" name="<?php echo esc_attr($this->get_field_name('template_id')); ?>">
				<option value="">— <?php esc_html_e('Select', 'puca'); ?> —</option>
				<?php
			...
			<a target="_blank" class="elementor-edit-template" href="<?php echo esc_url($edit_link); ?>">
				<i class="eicon-pencil"></i> <?php echo esc_html__('Edit Template', 'puca'); ?>
			</a>
		</p> 
		<?php
    }
